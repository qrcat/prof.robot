...
from utils.collision_network import HyperEnbedding, HyperNetwork, LipMLP, HyperDynamicNetwork
...
def main(args):
    ...
    if args.lipschitz:
        ...
        model = HyperDynamicNetwork(model.njnt, parents, remap_index, root_frame, init_s=2.3)
    model.cuda()
    ...
    trace = torch.jit.trace(model.cpu(), (torch.zeros(1, model.num_joints), torch.zeros(1, 100, 3)),)
