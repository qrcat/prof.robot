...
class JointSubsetData(data.Dataset):
    def __init__(self, joint: torch.Tensor, label: torch.Tensor, batch_size=100_000) -> None:
    ...
    def __getitem__(self, index) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        first_i, last_i = self.batch_size*index, min(self.batch_size*(index+1), len(self.labels))
...
class DrRobotData(data.Dataset):
    def __init__(self, data_path, t='label', shuffle=True) -> None:
        ...
        self.gts = torch.as_tensor(gts[reindex]).unsqueeze(-1)
    ...
    def load_distance(data_path):
        data = np.load(data_path)
        joint = data.get('joint')
        distance = data.get('dists')
        return joint, distance
    
    def get_split(self, split_ratio=[0.9, 0.1], batchsize=1_000_000) -> List[JointSubsetData]:
        ...
        for prev_f, next_f in zip(cumsum[:-1], cumsum[1:]):
            ...
            subset.append(
                JointSubsetData(self.joints[prev_i:next_i], self.gts[prev_i:next_i], batchsize)
            )
        return subset

    def __getitem__(self, index) -> Tuple[torch.Tensor, torch.Tensor]:
        return self.joints[index:index+self.batch_size], self.gts[index:index+self.batch_size]
...
if __name__ == "__main__":
    data_path = pathlib.Path("data/universal_robots_ur5e_collision")
    print("load pkl data from", data_path)

    pkl_files = list(data_path.glob('sample_*/data.npz'))
    tbar = tqdm(pkl_files, "Read PKL")

    with ThreadPool() as pool:
            OUT = pool.map(lambda x: (get_data(x, tbar)), pkl_files)
            pool.close()
            pool.join()
    ...
    dist_w__c = D.cpu().detach().numpy()
    dist_wo_c = np.zeros((len(new_quat_wo_c), 1), dtype=np.float32)

    joint = np.concatenate([new_quat_w__c.cpu().detach().numpy(), 
                            new_quat_wo_c.cpu().detach().numpy()], 0)
